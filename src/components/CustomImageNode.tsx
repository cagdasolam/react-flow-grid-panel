 ...
 */
const CustomImageNode = memo(({ data, isConnectable }: NodeProps) => {
  // Cast data to our CustomNodeData type for type safety
  const typedData = data as CustomNodeData & {
    width?: number;
    height?: number;
  };
  const { imageUrl, label, isRoot, width, height } = typedData;
